...
PhysicalDisplay::PhysicalDisplay() {
    ...
    const spi_bus_config_t bus_config = GC9A01_PANEL_BUS_SPI_CONFIG(SPI_LCD_PCLK, SPI_LCD_MOSI,
                                                                    SPI_LCD_H_RES * 80 * sizeof(uint16_t));
    ESP_ERROR_CHECK(spi_bus_initialize(SPI_LCD_HOST, &bus_config, SPI_DMA_CH_AUTO));

    ESP_LOGI(TAG, "Install panel IO");
    esp_lcd_panel_io_handle_t io_handle = nullptr;
    const esp_lcd_panel_io_spi_config_t io_config = GC9A01_PANEL_IO_SPI_CONFIG(SPI_LCD_CS,
                                                                               SPI_LCD_DC,
                                                                               nullptr, nullptr);
...
lv_obj_t *PhysicalDisplay::get_screen() {
    return screen_;
}
