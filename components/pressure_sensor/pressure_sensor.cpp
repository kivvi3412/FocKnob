...
PressureSensor::PressureSensor(gpio_num_t dout_pin, gpio_num_t sck_pin)
    ...
    ESP_ERROR_CHECK(esp_timer_start_periodic(scale_timer_, 100000));    // 100ms 读取一次重量 (10Hz)
}
...
// 读取 HX711 的原始数据
long PressureSensor::_hx711_read() {
    ...
    // 对读取的 24 位数据进行符号扩展
    if (Count & 0x800000) { // 判断最高位符号位
        Count |= 0xFF000000; // 负数, 进行符号扩展
    } else {
        Count &= 0x00FFFFFF; // 正数, 保留 24 位数据
    }
